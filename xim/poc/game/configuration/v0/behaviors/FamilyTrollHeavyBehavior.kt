...
import xim.poc.game.configuration.constants.mskillOverthrow_1639
import xim.poc.game.configuration.constants.mskillPotentLunge_1638
import xim.poc.game.configuration.constants.mskillRockSmash_1640
...
class FamilyTrollHeavyBehavior(actorState: ActorState): V0MonsterController(actorState) {
    ...
    override fun getSkills(): List<SkillId> {
        return super.getSkills() + if (isWeaponBroken()) {
            listOf(mskillRockSmash_1640, mskillOverthrow_1639)
        } else {
            listOf(mskillPotentLunge_1638)
        }
    }
